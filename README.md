...
以 SwiftUI 製作的 iPhone 健身 App，整合動作教學、可重複使用的訓練模板、逐組紀錄與休息計時。資料儲存在本機，不需要帳號或後端服務。動作縮圖優先從網路載入，等待或失敗時使用本機備援；動作教學與訓練紀錄可離線使用。
...
運動計時使用隨 App 打包的 Barlow Condensed SemiBold 客製字體，授權見 `Week 2/Resources/Barlow-OFL.txt`。

純白背景、淡色模板卡片與原生 Liquid Glass 分頁列，包含「首頁」「健身計劃」「運動紀錄」「收藏」。
...
「運動紀錄」分頁可查看歷史紀錄。修改或刪除模板不會影響既有紀錄及進行中的運動。
